# a_make_dateset_00.py
# changed from: a_make_dataset.py
# purpose: 정식개방데이터/이상행동/Train set 처리용
...
EVENTS = ["abandon","broken","fall","fight","fire","smoke","theft","weak_pedestrian"]
...
def parse_cvat_intervals(xml_path):
    ...
    s0 = int(task.findtext("start_frame","0") or "0")
    e0 = int(task.findtext("stop_frame","0")  or "0")

    # 1) <track>에서 라벨/프레임 수집
    start_frames = defaultdict(list)
    end_frames   = defaultdict(list)
    solid_runs   = defaultdict(list)  # outside=0 이 연속으로 이어진 일반 트랙 구간(라벨에 _start/_end 없는 경우)
    ...
    for tr in root.findall("track"):
        ...
        on = sorted([
            int(x.get("frame")) for x in tr.findall("box")
    ...
    # 3) start/end 페어로 간격 만들기 + 일반 트랙 구간 합치기
    ...
    # start/end 페어링 (가까운 end를 그 다음으로 매칭)
    for base in set(list(start_frames.keys()) + list(end_frames.keys())):
...
def main():
    ...
    for sub in os.listdir(args.video_root):
        ...
        for f in v_list:
            ...
            # sub내용 변경 : TS_03.이상행동_07.전도 => TL_03.이상행동_07.전도
            sub_xml = sub.replace("TS", "TL")
